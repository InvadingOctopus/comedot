...
* ComedotProjectSettings: Default framework settings resource for startup globals, music, turn-based timing, and debugging behavior.
...
* SpawnArea: Area2D spawn volume with a child SpawnTimer for spawning scenes at random positions inside the area.
...
* PlayerSpawnPosition: Marker2D scene that moves the current player Entity to the marker position on ready.
* SpawnEdge: CanvasLayer scene with eight edge and corner SpawnPoints plus four offscreen SpawnAreas, positioned and sized from the current viewport.
* SpawnPoint: Marker2D spawn scene with a child SpawnTimer for fixed-position spawning.
* SpawnTimer: Timer scene that periodically instantiates a configured scene.


## Scenes/Launch

* GameFrame: Main launch frame scene with debug background, camera behavior, global UI wiring, and the default main menu.
...
* CameraFollowingLabel: CanvasLayer label scene that follows the camera temporarily and removes itself offscreen.
