...
* LinearMotionComponent: Directly moves its Node2D parent using the parent's rotation or an override direction, with optional speed modification and cumulative-distance stopping or deletion. Good for simple high-count projectiles.
...
* TileBasedRandomMovementComponent: TileBasedControlComponentBase subclass that chooses random grid steps on Timer ticks and can reroll blocked moves. Requires TileBasedPositionComponent; use RandomInputComponent for lower-level random input generation.
...
* DropOnDeathComponent: Spawns a scene chosen from weighted paths when HealthComponent reaches zero and is configured to remove the entity, for loot, remains, or replacement creatures.
...
* DamageVisualComponent: Visual feedback for damage attempts received through DamageReceivingComponent, including damage absorbed by shields. Use HealthVisualComponent for Health Stat changes.
* HealthVisualComponent: Visual feedback for HealthComponent Stat changes, including damage and healing. Use DamageVisualComponent for incoming damage attempts.
...
Total listed: 123  
Generated by AI (Codex) on 2026-08-30
