...
* Spawner: Reusable Node script that prepares, validates, and instantiates a configured scene under a resolved parent, with optional spawning on ready, total/group limits, reentrancy protection, and setup/pre/post-spawn hooks for specialized subclasses. May also be attached to specialized Node types such as Timer.
...
* SpawnerStack: SpawnerSequenceBase subclass that pops scene paths from the end of a stack only after successful spawns, emits didPopFinalItem when depleted, and remains enabled so its list can be refilled.
...
Total listed: 146
Generated by AI (Codex) on 2026-07-30
