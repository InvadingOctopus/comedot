...
## Those methods are called on each [PhysicsComponentBase] by [method BodyEntity._physics_process] before and after the entity calls [method CharacterBodt2D.move_and_slide] (a function which must be called ONLY ONCE PER FRAME).
