...
@export_range(0, 1000, 5) var accelerationInAir:	float = 400.0

@export var shouldApplyFrictionInAir:				bool  = true ## Applies horizontal friction when not on a floor (not gravity).
@export_range(0, 5000, 5) var frictionInAir:		float = 200.0 ## Applies horizontal friction when not on a floor (not gravity).
...
@export_range(-1000, -10, 5) var jumpVelocity1stJump:		float = -350.0 ## NOTE: This should be a NEGATIVE value because a positive Y axis value means downwards.
@export_range(-1000, -10, 5) var jumpVelocity1stJumpShort:	float = -175.0 ## NOTE: This should be a NEGATIVE value because a positive Y axis value means downwards.
@export_range(-1000, -10, 5) var jumpVelocity2ndJump:		float = -300.0 ## NOTE: This should be a NEGATIVE value because a positive Y axis value means downwards.
