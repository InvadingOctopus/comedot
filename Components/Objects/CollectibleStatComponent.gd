...
func onCollectible_didCollect(collectibleComponent: CollectibleComponent, collectorEntity: Entity) -> int:
	...
	if shouldEmitBubble:
		GameplayResourceBubble.createForStatChange(stat, entity.get_parent(), Vector2(entity.position.x, entity.position.y - 16), shouldAppendStatName, shouldColorBubble) \
