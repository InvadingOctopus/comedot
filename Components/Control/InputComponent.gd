...
## [method handleInput] & [method processMonitoredInputActions] may be called manually at any time,
...
## The primary movmeent input from the combined horizontal + vertical axes. Includes the Left Joystick & the D-pad.
## NOTE: Diagonal movement is normalized, polled via [method Input.get_vector]. For "raw" axes, use [member horizontalInput] and [member verticalInput].
...
## NOTE: If the total input is a diagonal movement, then this axis is a "raw" value, polled via [method Input.get_axis]; for "normalized" diagonals, use [member movementDirection].
...
## NOTE: [member movementDirection] and other vectors are polled from the global [Input] state in [method handleInput].
...
func generateEvent(inputActionName: StringName, generateGlobalEvent: bool = false, pressed: bool = true, strength: float = 1 if pressed else 0, setAsHandled: bool = self.shouldSetEventsAsHandled) -> InputEvent:
	...
	# TODO: CHECK: Synthezie analog axes events like InputEventJoypadMotion etc.
	...
	# NOTE: If we're player-controlled, then handleInput() will be called by the global event → _input() or _unhandled_input() anyway.
	# If not, we need to manually handle our own generated event.
	if not generateGlobalEvent or not self.isPlayerControlled: self.handleInput(event)
