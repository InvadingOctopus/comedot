...
func readdAllAreas() -> void:
	...
	for overlappingArea in self.get_overlapping_areas():
		if shouldShowDebugInfo: Debug.printDebug(str("Adding ", overlappingArea), str(self))
...
func onAreaEntered(areaEntered: Area2D) -> void:
	...
	if not isEnabled: return
	if shouldShowDebugInfo: Debug.printDebug(str("Entered ", areaEntered), str(self))
...
func onAreaExited(areaExited: Area2D) -> void:
	...
	if shouldShowDebugInfo: Debug.printDebug(str("Exited ", areaExited), str(self))
