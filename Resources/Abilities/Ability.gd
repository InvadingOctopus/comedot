## Represents a special move, skill or ability that a player or another character may explicitly choose to perform. May have a cost and may require a target to be chosen.
...
@export var hasFiniteUses:	bool ## If `true`, then this Ability may be performed only for a number times equal to [member maximumUses]
@export var maximumUses:	int: ## The number of times this Ability may be performed, if [member hasFiniteUses]. Setting this property resets [member usesRemaining].
	set(newValue):
...
@export_range(0, 6000, 0.1) var cooldown: float = 0 ## The time in seconds (or fraction of a second) to wait before this Ability may be used again.

## The code to execute when this Ability is performed. See [Payload] for explanation and available options.
...
## The number of seconds remaining before this Ability may be used again.
## NOTE: This must be reduced by an [AbilityComponent] on every frame, because [Resource]s cannot perform any per-frame updates on their own.
...
## May be handled by game-specific UI to prompt the player to choose a target for this Ability.
## NOTE: If this Ability is to be performed via an [AbilityComponent]'s [method AbilityComponent.performAbility] then this signal will NOT be emitted; ONLY the Component's [signal AbilityComponent.didRequestTarget] is emitted.
...
## Returns whether this Ability is currently usable (i.e. off cooldown, have available uses) with the provided [Stat] and source/target.
...
## Returns `true` if the provided [param target] is valid or if this Ability does not need a target.
...
## Performs the Ability then if applicable, deducts the [member cost] from the [param paymentStat], decrements the [member usesRemaining] and starts the [member cooldownRemaining].
...
## TIP: PERFORMANCE: [param validateState] may be disabled by components or UI that perform their own validation, such as [AbilityComponent];
...
startCooldown:	bool = true) -> Variant:
	...
	if Tools.checkResult(payloadResult): # Must not be `null` and not `false` and not an empty collection
		...
		if target:
			var abilityTargetableComponent: AbilityTargetableComponent = target.getComponent(AbilityTargetableComponent, true) # findSubclasses to allow sublasses such as [AbilityReactionComponent]
			if  abilityTargetableComponent:
				abilityTargetableComponent.didTarget(self, source, payloadResult)
