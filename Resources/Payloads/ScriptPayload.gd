...
func executeImplementation(source: Variant, target: Variant) -> Variant:
	...
	if self.payloadScript:
		...
		# static func [payloadScriptMethodName](payload: Payload, source: Variant, target: Variant) -> Variant
		if debugMode: Debug.printLog(str(payloadScriptMethodName, "() source: ", source, ", target: ", target), str(self.logName, " payloadScript: ", self.payloadScript, " ", self.payloadScript.resource_path.get_file()), "", "pink") # A custom log to show the script as part of the logging object
