...
@export_range(8, 2000, 4) var acceleration:		float = 800
...
@export_range(0,     1000, 4) var verticalSpeed:				float = 320  ## The speed applied when the player presses the up or down controls.
@export_range(-1000, 1000, 4) var verticalVelocityDefault:		float = 0	 ## Apply a constant vertical thrust when is no player input. Positive = Down, Negative = Up

@export_range(-1000, 1000, 4) var verticalVelocityMin:			float = -320 ## Keeps the vertical velocity above or at this value. A positive value will maintain a downwards movement.
@export_range(-1000, 1000, 4) var verticalVelocityMax:			float = 320  ## Keeps the vertical velocity under or at this value. A negative value will maintain a upwards movement.
